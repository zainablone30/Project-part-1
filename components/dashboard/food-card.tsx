...
import { Heart, Star, Clock, Plus, Flame, Leaf, Award, AlertTriangle, Check } from "lucide-react"
...
interface FoodCardProps {
  ...
  nameUrdu?: string
  restaurant: string
  /** The area/neighbourhood the restaurant is in — used for map routing */
  ...
  description?: string
  allergens?: string[]
  /** Called after item is added — optional, for legacy callers */
  onAddToCart?: () => void
  onFavorite?: () => void
  className?: string
}
...
export function FoodCard({
  ...
  const handleAddToCart = () => {
    addItem({ foodId: id, name, price, image, restaurantName: restaurant, restaurantArea })
    onAddToCart?.()
    setAdded(true)
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-4/3 overflow-hidden">
        ...
        {warnings.length > 0 && (
          <motion.div
            className="absolute inset-0 z-10 flex flex-col justify-start p-4 bg-linear-to-b from-black/92 via-black/80 to-black/50"
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <div className="space-y-2">
              {warnings.map((w, i) => (
                <p key={i} className="text-xs text-amber-100 leading-snug">• {w}</p>
              ))}
            </div>
          </motion.div>
        )}
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-wrap gap-1.5">
          ...
          {aiRecommended && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="px-2 py-1 rounded-full bg-linear-to-r from-primary to-accent text-white text-xs font-semibold flex items-center gap-1"
            >
        ...
        {/* Add to Cart button — visible on hover, always works */}
        <motion.button
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: isHovered ? 1 : 0, y: isHovered ? 0 : 20 }}
          onClick={handleAddToCart}
          className={cn(
            "absolute bottom-3 right-3 z-20 px-4 py-2 rounded-full font-semibold text-sm flex items-center gap-2 shadow-lg transition-colors",
            added
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            ...
            {nameUrdu && <p className="text-sm text-muted-foreground">{nameUrdu}</p>}
          </div>
          <div className="flex items-center gap-1.5">
            ...
            {isSpicy && <span className="text-lg" title="Teekha hai!">🌶️</span>}
          </div>
        </div>
