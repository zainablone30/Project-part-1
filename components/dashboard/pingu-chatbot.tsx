...
type Phase = "idle" | "peekIn" | "runBack" | "happy"
...
const TRANSITION_BY_PHASE: Record<Phase, object> = {
  ...
  peekIn:  { type: "spring", stiffness: 120, damping: 18 },
  runBack: { type: "tween",  duration: 0.25, ease: "easeIn" },
...
function getGesture(phase: Phase): object {
  switch (phase) {
    ...
    case "peekIn":
      return { y: [0, -10, 0], transition: { duration: 0.8 } }
    case "happy":
      return {
        y:     [0, -35, 5, -22, 0, -10, 0],
...
export default function PinguChatbot() {
  ...
  useEffect(() => {
    ...
    async function step(p: Phase, ms: number): Promise<boolean> {
      setPhase(p)
      await sleep(ms)
      return !cancelRef.current
    }

    async function routinePeek() {
      setMood("waving")
      setBubbleText(pick(QUOTES))
      if (!await step("peekIn", 500))  return
      setShowBubble(true)
      if (!await step("peekIn", 2200)) return
      setShowBubble(false)
      setMood("idle")
      if (!await step("runBack", 350)) return
      setPhase("idle")
    }

    const first    = setTimeout(() => go(routinePeek), 3000)
    const interval = setInterval(() => go(routinePeek), 16000)
  ...
  return (
    <>
      ...
      {/* ── Pingu companion — peeks from LEFT edge ────────────────────────── */}
      <AnimatePresence>
        {!open && (
          <motion.div
            key="pingu-companion"
          ...
          >
            {/* Speech bubble — floats to the RIGHT of the peeking character */}
            <AnimatePresence>
              {showBubble && (
                <motion.div
                  key="bubble"
            ...
            <AnimatePresence>
              {phase === "happy" && (
                <motion.div
                  ...
                  transition={{ duration: 0.55 }}
                  className="pointer-events-none absolute -top-5 right-0 text-2xl select-none"
                >
                  ✨
                </motion.div>
              )}
